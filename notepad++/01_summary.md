
# notepad++{.chapter}

### content

- [Dark Theme in Notepad++](dark_theme_notepad++.md)
- [Disable automatic parens completion](disable_automatic_parens_completion.md)
- [Expand/Collapse keystrokes in Notepad++](expand_collapse.md)
- [How to navigate back in Notepad++](navigate_back.md)
- [Install TextFX plugin to notepad++](install_textfx_plugin.md)
- [Markdown in Notepad++](markdown_in_notepad.md)
- [Need a shortcut that inserts the current date/time?](todays_date.md)
- [Notepad++ command line help](command_line.md)
- [Plugins for Notepad++](plugins.md)
- [Resolved: There was an error downloading the plugin list](error_downloading_the_plugin_list.md)
- [Send to End Macro](sendtoEnd.md)
- [Telling NotePad++ About a New File Type](tell_notepad++_about_a_new_file_type.md)
- [Trim trailing and save](trim_trailing_and_save.md)
- [Use F4 to close documents](shortcut_close_document.md)
